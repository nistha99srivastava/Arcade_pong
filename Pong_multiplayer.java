...
public class Pong_multiplayer extends PApplet {
...
public void draw() {
  ...
  if (gameState == 0) 
  {                           //if we are in the pre-game menu
    textSize(40);
    text("Welcome to Pong", 200, 300);
    text("PRESS N", 200, 500);
    if (keyPressed  && key == 'n') {
      gameState = 1;
    }
  }
  ...
  if (gameState == 1) {                               //if we are in game
    ...
    if (ballPosX > p1PosX && ballPosX < p1PosX + paddleWidth && ballPosY > p1PosY && ballPosY < p1PosY + paddleHeight) {
      ballVelocityX = ballVelocityX * -1.4f;
    }
    ...
    if (p1PosY + paddleHeight > height) {        //so paddle does not exit screen
...
public void keyReleased() {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "Pong_multiplayer" };
    if (passedArgs != null) {
